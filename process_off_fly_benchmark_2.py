...
from WhiteLib_test2 import T_sinc, merge_time_channel_arrays, find_coincidences, Modes_separator, compact_results
...
def process_measurement(measurement, photons):
    ...
    # times_box_2_real = times_box_2[channels_box_2 != trigger_channel]
    # x1=np.arange(len(times_box_1))
    # x2=np.arange(len(times_box_2_real))
    # plt.plot(x1, times_box_1, label='Box 1')

    # plt.plot(x2, times_box_2_real, label='Box 2')
...
if __name__ == "__main__":
    ...
    files.sort()
    # === PROCESSA FILE ===
    for file in tqdm(files[0:1], desc="Processing files"):
